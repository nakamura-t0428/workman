...
trait ProjectT extends Driver with UserT with CompanyT {
  ...
  class Projects(tag:Tag) extends Table[Project](tag, "PROJECT") {
    ...
    def compId = column[String]("COMP_ID", O.SqlType("CHAR(24)"))
    ...
    def * = (prjId, name, description, compId, ownerId, regDate, disabled, permManager, permMember, permGuest) <> (Project.tupled, Project.unapply)
  }
